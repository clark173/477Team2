...
void InitUART2( void )
{
    ...
    U1STAbits.URXISEL = 1;  //Bits6,7 Int. on character recieved
...
void SoftwareDebounce( void )
{
    ...
    if( PORTCbits.RC9 == FALSE )
    {
        if( S6Flag == FALSE )
        {
            S6Flag = TRUE;
            printf("You just pressed RC9\r\n");
        }
    }
    else
    {
    ...
    if( PORTFbits.RF7 == FALSE )
    {
        if( s5flag == FALSE )
        {
            s5flag = TRUE;
            printf("You just pressed RF7\r\n");
        }
    }
    else
    {
